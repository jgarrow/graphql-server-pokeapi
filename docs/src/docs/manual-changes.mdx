...
2. Manually update the `pokemon_id` entries for the entries that belong to an alternate form instead of the default form. You can use this query to grab all of the English "flavor texts" based on the `pokemon_species_id`. Then you can manually update the `pokemon_id` entries in the necessary rows.
